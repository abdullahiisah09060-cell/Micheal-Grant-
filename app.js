...
export function requireAuth(callback) {
    ...
    onAuthStateChanged(auth, async (user) => {
        ...
        listenUserData(user.uid, (userData) => {
            ...
            if (config?.maintenanceMode && userData.role !== 'admin') {
                document.body.innerHTML = `
                    <div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;background:var(--bg);padding:40px;text-align:center;">
                        ...
                        <p style="color:var(--text-secondary);max-width:400px;">The SBA portal is temporarily offline for scheduled system updates. Please check back shortly.</p>
                    </div>`;
                hideLoader();
                return;
            }
            ...
            if (userData.accountStatus === 'suspended') {
                document.body.innerHTML = `
                    <div class="status-screen">
                        ...
                        <p class="status-screen__message">Your access has been revoked by the SBA Compliance team. Contact support for assistance.</p>
            ...
            // Success: Call back with data, then hide loader
...
export function redirectIfLoggedIn() {
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            if (userData.data()?.role === 'admin') window.location.href = 'admin-portal.html';
...
export function renderStatusBadge(s) {
    ...
    const icons = { 'IDLE':'fa-clock', 'PENDING':'fa-spinner fa-spin', 'UNDER_REVIEW':'fa-magnifying-glass', 'APPROVED':'fa-circle-check', 'REJECTED':'fa-circle-xmark', 'CONFIRMED':'fa-shield-check' };
    return `<span class="badge badge--${cls}"><i class="fa-solid ${icons[s] || 'fa-info'}"></i> ${s}</span>`;
}
...
export function buildPageHeader({ title, subtitle, backUrl }) {
    ...
    header.innerHTML = `
        <div class="page-header">
            ...
            <div style="flex:1"><h1 class="page-header__title">${title}</h1>${subtitle ? `<p class="page-header__sub">${subtitle}</p>` : ''}</div>
...
export function buildSidebar({ activeId, userData }) {
    ...
    sidebar.innerHTML = `
        ...
        <nav class="app-sidebar__nav">
            ...
            <a href="apply.html" class="nav-item ${activeId==='apply'?'active':''}"><i class="fa-solid fa-file-signature"></i> Apply</a>
            <a href="kyc.html" class="nav-item ${activeId==='kyc'?'active':''}"><i class="fa-solid fa-id-card"></i> Verify ID</a>
            ...
            <a href="support.html" class="nav-item ${activeId==='support'?'active':''}"><i class="fa-solid fa-comments"></i> Chat <span class="nav-item__badge notif-badge">0</span></a>
            <a href="settings.html" class="nav-item ${activeId==='settings'?'active':''}"><i class="fa-solid fa-gear"></i> Settings</a>
        </nav>
        <div class="app-sidebar__footer">
            <div class="flex items-center gap-4 mb-4">
                <div class="nav-user-avatar" style="width:36px;height:36px;border-radius:50%;background:var(--navy);color:white;display:flex;align-items:center;justify-content:center;font-weight:700;">${userData.fullName.charAt(0)}</div>
            ...
            <button class="nav-item" id="btnLogout" style="width:100%;text-align:left;border:none;background:none;cursor:pointer;"><i class="fa-solid fa-right-from-bracket"></i> Sign Out</button>
        </div>`;
    document.getElementById('btnLogout').onclick = () => {
        showConfirm({ title: 'Sign Out', message: 'Log out of your secure session?', onConfirm: () => auth.signOut().then(() => window.location.href = 'login.html') });
    };
...
export function buildDock({ activeId }) {
    ...
    dock.innerHTML = `
        ...
        <a href="support.html" class="dock-item ${activeId==='support'?'active':''}"><i class="fa-solid fa-headset"></i><span>Chat</span></a>
